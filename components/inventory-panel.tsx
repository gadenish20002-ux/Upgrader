    ...
    <div className="h-full flex flex-col">
      ...
      <div className="relative flex-1 p-3 rounded-b-xl bg-[#16171a] overflow-hidden min-h-[500px] lg:min-h-[560px] flex flex-col">
        {!state.loggedIn ? (
          ...
          <div className="relative flex flex-1 min-h-[400px] w-full items-center justify-center">
            {/* Empty state per the reference: SHARP full item cards (price + coins,
                weapon, name) behind a backdrop-blur overlay that covers the WHOLE
                panel. The parent has p-3, so the overlay uses -inset-3 — otherwise
                the blur stops at the padding and looks cut off. */}
            {!isSpinning && (
              <div className="pointer-events-none absolute inset-0 grid grid-cols-3 content-start gap-1 overflow-hidden px-1.5 select-none lg:grid-cols-5 lg:gap-1.5">
                {Array.from({ length: 50 }).map((_, i) => {
                  ...
                  const SHADOWS = ["ak47", "usp", "famas", "knife"]
                  const shadow = SHADOWS[i % 4]
                  return (
                    <div key={i} className="bg-block flex h-[5rem] items-center justify-center overflow-visible rounded-md bg-[length:85%_85%] bg-center bg-no-repeat lg:h-[6.75rem]" style={{ backgroundImage: `url('/assets/item-shadow/${shadow}.png')` }}>
                        ...
                        <div className="bg-block tablet:bg-size-[50%] relative flex h-full w-full items-center justify-center rounded-md bg-cover bg-[length:2.5rem] bg-center bg-no-repeat lg:bg-[length:50%]" style={{ backgroundImage: "url('/cdn/fa/images/light-gray-logo.svg')" }}>
                          <div className="absolute z-[2] flex flex-col items-end justify-center space-x-0.5 top-1.5 right-1.5">
                            <div className="flex items-center justify-center space-x-0.5">
                              <span className="font-tektur text-gradient-yellow text-[0.5rem] lg:text-[0.625rem] font-bold text-white">{formatPrice(sk.price)}</span>
                              <img alt="" className="h-2 w-2 lg:h-2.5 lg:w-2.5" src="/cdn/fa/icons/coins.svg" />
                            </div>
                          </div>
                          ...
                          <div className="absolute left-1/2 z-[2] flex w-full max-w-[80%] -translate-x-1/2 flex-col items-center justify-center text-center bottom-1.5">
                            <span className="text-gray font-semibold text-xxxxs">{formatWeaponName(sk.weapon)}</span>
                            <span className="text-white text-xxxs font-tektur max-w-full truncate font-bold lg:text-xxs">{formatSkinName(sk.name)}</span>
                          </div>
            ...
            {/* Blur overlay over the WHOLE panel (reaches past the p-3 padding). */}
            <div className="absolute -inset-3 z-[3] rounded-b-xl bg-[rgba(25,25,27,0.10)] backdrop-blur-xs"></div>
